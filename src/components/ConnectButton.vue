...
<template>
    <Popover class="relative">
        <PopoverButton v-if="bc.loaded" class="h-7 text-white px-1 font-thin items-center flex text-xs rounded-full hover:bg-black hover:text-white focus:outline-none focus:ring-1 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" @click="onClicked()">
        ...
        <PopoverButton v-else-if="bc.connectionError" class="bg-red-500 px-1 items-center flex rounded-md focus:outline-none focus:ring-2 hover:text-black  focus:ring-offset-1 focus:ring-offset-gray-800 focus:bg-red-200 focus:ring-white" @click="onClicked()">
        ...
        <PopoverButton v-else-if="!bc.loaded" class="bg-blue-500 px-1 items-center flex rounded-md focus:outline-none focus:ring-2 hover:text-black focus:ring-offset-1 focus:ring-offset-gray-800 focus:bg-indigo-300 focus:ring-white" v-on:click.prevent="onClicked()">
            <StatusOnlineIcon class="text-white hover:text-black h-5 w-5" aria-hidden="true" />
            <span class="text-white hover:text-black ml-1 mr-2">Start</span>
        </PopoverButton>

        <transition name="fadeslide">
            <PopoverPanel v-if="bc.loaded" 
                          class="origin-top-right absolute w-screen sm:w-max opacity-90 p-5 text-sm -right-3 sm:right-0 mt-2 sm:mt-3 h-screen sm:h-auto sm:rounded-md shadow-md bg-black text-white z-50">
            ...
            <PopoverPanel v-else-if="bc.connectionError" class="origin-top-right overflow-hidden absolute w-screen sm:w-auto p-5 text-sm right-0 mt-2 rounded-lg shadow-lg bg-white text-black ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
